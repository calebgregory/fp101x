...
-- Here is an example...
  --  > 40128888881881
  --  1. In order to start with the rightmost digit, we reverse the list of digits:
  --     > [1, 8, 8, 1, 8, 8, 8, 8, 8, 8, 8, 8, 2, 1, 0, 4]
  --     and double every second digit:
  --     > [1,16, 8, 2, 8,16, 8,16, 8,16, 8,16, 2, 2, 0, 8]
  --  2. We sum all of the digits of the resulting list above. (Note: we must split
  --     the elements of the list into their digits - e.g., 16 becomes [1,6])
  --     > result: 90
  --  3. Finally, we calculate 90 mod 10
  --     > result: 0.
